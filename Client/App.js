...
class App extends Component {
  ...
  sendSignal = async (code) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (found) return;
        throw new Error("Timeout: Time Exceeded");
      }, 5000);
  ...
  render() {
    return (
      <View style={styles.container}>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('0')}>
          <Text>Off</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('1')}>
          <Text>Chase</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('2')}>
          <Text>Alternating RGB</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('3')}>
          <Text>Random Fade</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('4')}>
          <Text>Rainbow</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('5')}>
          <Text>Rainbow With Glitter</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => this.sendSignal('6')}>
          <Text>BPM</Text>
        </TouchableOpacity>
        <View>
